...
def main():
    ...
    #print('a')
    starttime =time.time()   #actual time by laptop does no start with zero hance
    i = 0
    ...
    fig3.show()
    while True:
        #Reading the data and saving the reading
        ...
        #print(Rcap)
            ...
            plt.figure(1)
            plt.plot(T,np.array(Thetat)*180/pi)
            fig1.canvas.draw()
            plt.xlim(0,60)
            plt.ylim(-180,180)
            
            plt.figure(2)
            plt.plot(T,np.array(Psit)*180/pi)
            fig2.canvas.draw()
            plt.xlim(0,60)
            plt.ylim(-180,180)
            
            plt.figure(3)
            plt.plot(T,np.array(Phit)*180/pi)
            fig3.canvas.draw()
            plt.xlim(0,60)
            plt.ylim(-180,180)
