...
Kp = 10  #correction term coefficient 
Ki = 5   #Bias update coefficient
Ka = 0.95  #accelerometer term coefficient in correction
...
Rdes = np.matrix([ [0.1470158, -0.3617544,  0.9206086],
   [0.6916548,  0.7029465,  0.1657706],
  [-0.7071068,  0.6123725,  0.3535534] ])                 #the required matrix to be tracked 
                                                          #corresponding euler angles are 60,45,78 degrees in ZYX format
  
#Rdes = Rdes.transpose(Rdes)
...
def main():
    df = pd.read_csv('/home/dell/Project/csv/data.csv') 
    data = np.array(df)
    ...
    for i in range(data.shape[0]-1):
        if i == 0:
            ...
            vg = np.transpose(Rdes)*vg0
            vg = np.array([vg[0,0],vg[1,0],vg[2,0]])
            vm = np.transpose(Rdes)*vm0
            vm = np.array([vm[0,0],vm[1,0],vm[2,0]])
...
#                print(Rcap)
    ...
    print(vmcap,vm,"\n")
    print(vgcap,vg,"\n")
    Rtilde = np.transpose(Rcap)*Rdes
    print(Rtilde,"\n")
    print(Rtilde.trace(),"\n")
    PaRtilde = (Rtilde - np.transpose(Rtilde))/2
    print(PaRtilde)
