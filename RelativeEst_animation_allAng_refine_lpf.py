...
# Add labels
vgr_lpf1, vgr_lpf2, vmr_lpf1, vmr_lpf2 = 0, 0, 0, 0
i = 0
prevtime1 = 0
prevtime2 = 0
dotR = 0
...
def calculateAng():
        ...
        global vgr_lpf1, vgr_lpf2, vmr_lpf1, vmr_lpf2
...
#                print(Rcap)
            ...
            vgr1 = reading1[1:4]
            vgr_lpf1 = alfag*vgr_lpf1 + (1-alfag)*vgr1
            vg1 = vgr_lpf1/(np.linalg.norm(vgr_lpf1))

            #vg = np.transpose(vg)
            vmr1 = reading1[7:]
            vmr_lpf1 = alfam*vmr_lpf1 + (1-alfam)*vmr1
            vm1 = vmr_lpf1/(np.linalg.norm(vmr_lpf1))
            #vm = vmr                             #for i = 0 low pass filter reading will be the actual reading
            vgcap1 = np.transpose(Rcap1)*vg0
            vgcap1 = np.array([vgcap1[0,0],vgcap1[1,0],vgcap1[2,0]])
            ...
            vgr2 = reading2[1:4]
            vgr_lpf2 = alfag*vgr_lpf2 + (1-alfag)*vgr2
            vg2 = vgr_lpf2/(np.linalg.norm(vgr_lpf2))

            #vg = np.transpose(vg)
            vmr2 = reading2[7:]
            vmr_lpf2 = alfam*vmr_lpf2 + (1-alfam)*vmr2
            vm2 = vmr_lpf2/(np.linalg.norm(vmr_lpf2))
            #vm = vmr                             #for i = 0 low pass filter reading will be the actual reading
            vgcap2 = np.transpose(Rcap2)*vg0
            vgcap2 = np.array([vgcap2[0,0],vgcap2[1,0],vgcap2[2,0]])
